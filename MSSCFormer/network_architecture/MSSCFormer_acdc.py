...
from MSSCFormer.network_architecture.layers import LayerNorm
from MSSCFormer.network_architecture.dynunet_block import get_conv_layer, ResBlock, OutBlock
...
class SC_TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv52 = ResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
...
class Encoder(nn.Module):
    def __init__(self, input_size=[16 * 40 * 40, 8 * 20 * 20, 4 * 10 * 10, 2 * 5 * 5], dims=[32, 64, 128, 256],
        ...
        self.scale_img1 = nn.AvgPool3d((1, 4, 4), (1, 4, 4))
        self.scale_img2 = nn.AvgPool3d((2, 2, 2), (2, 2, 2))

        self.downsample_layers.append(stem_layer)
        self.isri_layers.append(ISRI(dims[0], dims[0]))
        for i in range(3):
            self.conv.append(nn.Sequential(nn.Conv3d(1, out_channels=dims[i], kernel_size=3, stride=1, padding=1)))
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(SC_TransformerBlock(input_size=input_size[i], hidden_size=dims[i],
    ...
    def __init__(
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(SC_TransformerBlock(input_size=out_size, hidden_size=out_channels,
...
class InterScaleAttention(nn.Module):
    ...
    def forward(self, x):
        B, num_blocks_d, num_blocks_h, num_blocks_w, _, C = x.shape
...
class ISTB(nn.Module):
    ...
    def forward(self, x):
        x = [self.fc_module[i](item.permute(0, 2, 3, 4, 1)) for i, item in enumerate(x)]
...
class MSSCFormer(SegmentationNetwork):
    ...
    def __init__(
        ...
        # define encoder
        self.encoder = Encoder(dims=dims, depths=depths, num_heads=num_heads)

        # define inter_scale transformer bridge
    ...
    def forward(self, x_in):
        ...
        fusions = []
        for i in range(self.num_module):
            skip = self.fusion_module[i](torch.cat((bridged_states[i], encoded_states[i]), dim=1))
            fusions.append(skip)
