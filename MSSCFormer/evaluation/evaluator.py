...
from MSSCFormer.evaluation.metrics import ConfusionMatrix, ALL_METRICS
...
def MSSCFormer_evaluate_folder():
    import argparse
...
if __name__ == '__main__':
    training_output_folder1 = "G:/CY\CodesOfCy/new/MSSCFormer-main/DATASET/MSSCFormer_trained_models/MSSCFormer/3d_fullres/Task001_ACDC/MSSCFormerTrainerV2_MSSCFormer_acdc__MSSCFormerPlansv2.1"
    plans = load_pickle(join(training_output_folder1, "plans.pkl"))
    output_folder = "G:/CY/CodesOfCy/new/MSSCFormer-main/DATASET/MSSCFormer_trained_models/MSSCFormer/ensembles/Task001_ACDC/ensemble_3d_fullres__MSSCFormerTrainerV2_MSSCFormer_acdc__MSSCFormerPlansv2.1--3d_fullres__MSSCFormerTrainerV2_MSSCFormer_acdc__MSSCFormerPlansv2.1"
    validation_folder = "validation_raw"
    f = 0
    validation_folder_net1 = join(training_output_folder1, "fold_%d" % f, validation_folder)
    patient_identifiers1_npz = [i[:-4] for i in subfiles(validation_folder_net1, False, None, 'npz', True)]#npz格式的才行
    out_files=[]
    gt_segmentations=[]
    folder_with_gt_segs = "G:/CY/CodesOfCy/new/MSSCFormer-main/DATASET/MSSCFormer_raw/MSSCFormer_raw_data/Task01_ACDC/labelsTr"
    for p in patient_identifiers1_npz:
        out_files.append(join(output_folder, p + ".nii.gz"))
        gt_segmentations.append(join(folder_with_gt_segs, p + "_gt.nii.gz"))

    aggregate_scores(tuple(zip(out_files, gt_segmentations)), labels=plans['all_classes'],
                     json_output_file=join(output_folder, "summary.json"), json_task="acdc",
                     json_name="acdc" + "__" + output_folder.split("/")[-1], num_threads=1)